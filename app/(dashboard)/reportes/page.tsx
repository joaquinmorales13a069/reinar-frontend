...
export default function ReportesPage() {
  ...
  // y sólo permite inventario a LOGISTICA/OPERADOR. VISUALIZADOR no tiene acceso
  // a ninguno, así que lo regresamos al dashboard.
  const puedeVerExportables = rol === 'ADMIN' || rol === 'GERENTE';
  const puedeVerInventario  = puedeVerExportables || rol === 'LOGISTICA' || rol === 'OPERADOR';
  ...
  const visibles = TIPOS.filter((t) => (t.id === 'inventario' ? puedeVerInventario : puedeVerExportables));
    ...
    <div>
      ...
      {!puedeVerExportables && puedeVerInventario && (
        <p className="text-xs text-tx-3 mt-6">
          Tu rol sólo permite consultar el reporte de inventario. Para los reportes financieros y operativos contactá a un administrador.
        </p>
      )}
    </div>
  );
